...
public class Solution {
    ...
    public static void main(String[] args) throws FileNotFoundException {
        ...
        int totalNumberOfElements = Integer.parseInt(firstLineInputArray[0]);

        int[] allElementsArray = new int[totalNumberOfElements];
    ...
    static int maxDifference(int[] a) {

        {
            ...
            int minimumValueSoFar = a[0];

            for (int i = 1; i < a.length; i++) {
                int deltaWithMinimumElement = a[i] - minimumValueSoFar;
                if (deltaWithMinimumElement > maximumDifference)
                    maximumDifference = deltaWithMinimumElement;
                if (a[i] < minimumValueSoFar) {
                    minimumValueSoFar = a[i];
                }
            }
